...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['profile_id'])) {
    ...
    if (
        ...
        $profileController->updateProfile($id, $first_name, $family_name, $phone_number, $region, $city, $bio, $current_position, $education, $subscription_id, $auth, $acc_verif, $bday, $gender, $profile_photo_data, $profile_cover_data);
    }
    ...
    elseif (!empty($_FILES['profile_photo']['name']) && $_FILES['profile_photo']['error'] === 0) {
        ...
        $profileController->updateProfileWithoutImage($id, $first_name, $family_name, $phone_number, $region, $city, $bio, $current_position, $education, $subscription_id, $auth, $acc_verif, $bday, $gender,);
    }
