...
// Vérifier si l'identifiant du commentaire à supprimer est passé dans l'URL
if(isset($_GET['id'])) {
    // Supprimer le commentaire avec l'identifiant passé dans l'URL
    $commentaireController->deleteCommentaire($_GET["id"]);
            ...
            <div class="row">
              ...
              >
                <div class="container">
                  <!-- STARTHERE -->
  <div class="container">
    <h1 class="text-center mb-4">Liste des commentaires</h1>
    <div class="text-center mb-3">
        <a href="add_commentaire.php" class="btn btn-primary">Ajouter un commentaire</a>
    </div>
    <div class="row">
      ...
      <div class="col-md-6">
        <div class="card mb-4">
          <div class="card-body">
            ...
            <h5 class="card-title">Commentaire de <?php echo $commentaire['auteur']; ?></h5>
            <p class="card-text"><strong>Contenu:</strong> <?php echo $commentaire['contenu']; ?></p>
            <p class="card-text"><strong>Date de commentaire:</strong> <?php echo $commentaire['date_commentaire']; ?></p>

            <p>Likes: <span id="likes_count_<?php echo $commentaire['id_commentaire']; ?>"><?php echo $commentaire['likes']; ?></span></p>
            <p>Dislikes: <span id="dislikes_count_<?php echo $commentaire['id_commentaire']; ?>"><?php echo $commentaire['dislikes']; ?></span></p>
<div class="btn-group">
  <button onclick="updateLikesDislikes(<?php echo $commentaire['id_commentaire']; ?>, 'like')" style="background-color: lightgray; border: none; padding: 5px 10px; margin-right: 5px;">Like</button>
  <button onclick="updateLikesDislikes(<?php echo $commentaire['id_commentaire']; ?>, 'dislike')" style="background-color: lightgray; border: none; padding: 5px 10px; margin-right: 5px;">Dislike</button>
</div>
  ...
  <script>
function updateLikesDislikes(commentId, type) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            var response = JSON.parse(this.responseText);
            document.getElementById('likes_count_' + commentId).innerHTML = response.likes;
            document.getElementById('dislikes_count_' + commentId).innerHTML = response.dislikes;
        }
    };
    xhttp.open("POST", "update_likes_dislikes.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send("comment_id=" + commentId + "&type=" + type);
}
</script>


  <script>
    function confirmDelete(commentaireId) {
      if (confirm("Êtes-vous sûr de vouloir supprimer ce commentaire ?")) {
        window.location.href = "liste_commentaire.php?id=" + commentaireId;
      }
    }
  </script>
