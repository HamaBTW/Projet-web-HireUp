...
$id_artc_options = $commentaireC->generateArticleOptions();
...
<html lang="en">
          ...
          <div class="container">
            <div class="row">
              ...
              >
                <div class="container">
  <div class="container">
    <a href="./teamplate/index.php" class="btn btn-secondary btn-return"><i class="fas fa-arrow-left mr-2"></i>Retour</a>
    <div class="card">
      <div class="card-body">
        <h1 class="title">Ajouter un nouveau commentaire</h1>
        <form action="add_commentaire.php" method="post" onsubmit="return validateForm()">
          <div class="form-group">
          ...
          <div class="form-group">
            <label for="auteur"><i class="fas fa-user mr-2"></i>Auteur :</label>
            <input type="text" class="form-control" id="auteur" name="auteur">
            <span id="auteur_error" class="error-message"></span>
          </div>
          <div class="form-group">
            <label for="contenu"><i class="fas fa-comment mr-2"></i>Contenu :</label>
            <textarea class="form-control" id="contenu" name="contenu" rows="4"></textarea>
            <span id="contenu_error" class="error-message"></span>
          </div>
          <div class="form-group">
            <label for="date_commentaire"><i class="fas fa-calendar-alt mr-2"></i>Date de commentaire :</label>
            <input type="date" class="form-control" id="date_commentaire" name="date_commentaire" >
            <span id="date_commentaire_error" class="error-message"></span>
          </div>
          <button type="submit" class="btn btn-primary"><i class="fas fa-plus-circle mr-2"></i>Ajouter</button>
        </form>
      </div>
    </div>
  </div>
  ...
  <script>
    function validateForm() {
      var id_articleInput = document.getElementById('id_article');
      var auteurInput = document.getElementById('auteur');
      var contenuInput = document.getElementById('contenu');
      var date_commentaireInput = document.getElementById('date_commentaire');
      var id_articleError = document.getElementById('id_article_error');
      var auteurError = document.getElementById('auteur_error');
      var contenuError = document.getElementById('contenu_error');
      var date_commentaireError = document.getElementById('date_commentaire_error');
      var id_articleValue = id_articleInput.value;
      var auteurValue = auteurInput.value;
      var contenuValue = contenuInput.value;
      var date_commentaireValue = date_commentaireInput.value;

      // Réinitialise les messages d'erreur
      id_articleError.textContent = '';
      auteurError.textContent = '';
      contenuError.textContent = '';
      date_commentaireError.textContent = '';

      // Valide l'ID de l'article
      if (!id_articleValue.trim()) {
        id_articleError.textContent = "L'ID de l'article est requis.";
        return showAlert('Veuillez remplir tous les champs.');
      }

      // Valide l'auteur
      if (!auteurValue.trim()) {
        auteurError.textContent = "L'auteur est requis.";
        return showAlert('Veuillez remplir tous les champs.');
      }

      // Valide le contenu
      if (!contenuValue.trim()) {
        contenuError.textContent = "Le contenu est requis.";
        return showAlert('Veuillez remplir tous les champs.');
      }

      // Valide la date de commentaire
      if (!date_commentaireValue.trim()) {
        date_commentaireError.textContent = "La date de commentaire est requise.";
        return showAlert('Veuillez remplir tous les champs.');
      }
